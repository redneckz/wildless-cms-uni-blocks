...
export interface HeaderRightBlockProps {
  className?: string;
}

export const HeaderRightBlock = JSX<HeaderRightBlockProps>(({ className }) => {
  return (
    <div
      className={`flex items-center ${className || ''}`}
    >
      <a
        className={`mr-5 font-sans text-pimary text-sm hover:text-brand no-underline`}
        href="#"
      >
        Москва
      </a>
      <a
        className={`mr-7 font-sans text-pimary text-sm hover:text-brand no-underline`}
        href="#"
      >
        Офисы и банкоматы
      </a>
      <button
        type="button"
        className={`mr-5 border-0 p-0 w-[24px] text-pimary hover:text-brand bg-inherit cursor-pointer`}
      >
        {renderLoupeIcon()}
      </button>
      <button
      ...
      <button
        type="button"
        className={`mr-5 border-0 p-0 w-[32px] text-pimary bg-inherit cursor-pointer`}
      >
        {renderGridIcon()}
      </button>
    </div>
...
const renderGridIcon = () => {
  ...
  return (
    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 32 32">
      <rect y=".5" width="32" height="32" rx="16" fill="#F0F4F0" />
      <path
        {...params}
